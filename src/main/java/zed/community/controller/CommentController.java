...
import zed.community.entity.Comment;
import zed.community.entity.DiscussPost;
import zed.community.entity.Event;
...
public class CommentController implements CommunityConstant {
    ...
    public String addComment(@PathVariable("discussPostId") int discussPostId, Comment comment) {
        ...
        if (comment.getEntityType() == ENTITY_TYPE_POST) {
            ...
            DiscussPost target2 = discussPostService.findDiscussPostById(discussPostId);
            event.setEntityUserId(target.getUserId());
        } else if (comment.getEntityType() == ENTITY_TYPE_COMMENT) {
            Comment target = commentService.findCommentById(comment.getEntityId());
            Comment target2 = commentService.findCommentById(discussPostId);
            event.setEntityUserId(target.getUserId());
        }
